  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 md:px-6 md:py-10 fade-in">
      <AdminPageHeader
        ...
        actions={<Button size="sm" onClick={() => setCreateOpen(true)}><Plus className="mr-1 size-4" />{t("create")}</Button>}
      />

      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total notifikasi" value={String(stats.total)} accent="primary" />
        <StatCard label="Terkirim" value={String(stats.sent)} accent="secondary" />
        <StatCard label="Terjadwal" value={String(stats.scheduled)} accent="tertiary" />
        <StatCard label="Draft" value={String(stats.draft)} accent="primary" />
      </div>
