...
func TestOrderPaidHandler_PhysicalItem_ShipPendingNoEntitlement(t *testing.T) {
	for _, productType := range []string{"merchandise", "medal"} {
			...
			repo := &mockRepository{
				claimOutboxEventsFn: func(ctx context.Context, limit int) ([]model.OutboxEvent, error) {
					payload, _ := json.Marshal(OrderPaidPayload{
						OrderID: orderID,
						Items:   []OrderItemMini{{ProductID: productID, ProductType: productType}},
					})
